...
Since `Block` is an interface you have to use spread syntax in your queries if you want to target specific block type, for example for `core/paragraph` block, the representing graphql object type is called `CoreParagraphBlock` and the syntax for querying is:
